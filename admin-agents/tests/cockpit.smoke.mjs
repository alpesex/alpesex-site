...
for (const id of ['agent-network', 'decision-list', 'journal-body', 'decision-dialog', 'input-form', 'input-body', 'wake-coordinator', 'wake-status']) {
...
assert.match(js, /action:'wake_coordinator'/, 'The manual wake button must use the protected cockpit action');
assert.match(api, /new RateLimiter\(\$pdo\).*?coordinator_wake/s, 'Manual wake requests must be rate limited');
