const CACHE = 'cpmp-asm-web-6.0.0-2';
...
self.addEventListener('activate', event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => (key.startsWith('cpmp-asm-web-') || key.startsWith('cpmp-asm-mobile-')) && key !== CACHE).map(key => caches.delete(key)))).then(() => self.clients.claim())));
