const CACHE = 'cpmp-asm-web-5.4.19-1';
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request).then(response => {
      if (response.ok && !response.redirected && new URL(response.url).pathname.startsWith('/application/')) caches.open(CACHE).then(cache => cache.put('/application/', response.clone()));
      return response;
